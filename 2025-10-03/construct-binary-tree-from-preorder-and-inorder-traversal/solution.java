...
class Solution {
    ...
    private TreeNode solve(int[]preorder,int[]inorder,int inorderStart,int inorderEnd){
        if(inorderStart>inorderEnd || index>preoder.length){
            return null;
        }
        ...
        ans.left=solve(int[]preorder,int[]inorder,0,pos-1);
        ans.right=solve(int[]preorder,int[]inorder,pos+1,inorder.length-1);
        return ans;
